...
for filename in ./config/federatedirectives/*.yaml; do
  ./bin/kubefed2 --kubeconfig ${WORKDIR}/kubeconfig federate type -f "${filename}" --federation-namespace="${NS}" --host-cluster-context federation -o yaml > ${CHART_FEDERATED_PROPAGATION_DIR}/$(basename $filename)
done
