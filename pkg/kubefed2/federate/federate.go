...
func (j *federateResource) Complete(args []string) error {
	...
	// TODO(irfanurrehman): Add special handling to federate namespaces.
	if j.typeName == "namespaces" {
		return errors.New("Federate namespaces is not supported yet.")
	}
...
func lookUpTypeDetails(config *rest.Config, qualifiedTypeName ctlutil.QualifiedName) (*fedv1a1.FederatedTypeConfig, error) {
	...
	typeConfig, err := fedClient.CoreV1alpha1().FederatedTypeConfigs(qualifiedTypeName.Namespace).Get(qualifiedTypeName.Name, metav1.GetOptions{})
...
func createFedResource(hostConfig *rest.Config, typeConfig *fedv1a1.FederatedTypeConfig, template *unstructured.Unstructured) error {
	...
	if err != nil {
		return errors.Wrapf(err, "Error setting template into %q of type %s", qualifiedName, fedkind)
	}
	...
	if err != nil {
		return errors.Wrapf(err, "Error setting placement into %q of type %s", qualifiedName, fedkind)
	}

	_, err = fedClient.Resources(resourceNamespace).Create(fedResource, metav1.CreateOptions{})
	if err != nil {
		return errors.Wrapf(err, "Error creating Federate resource %q of kind %s", qualifiedName, fedkind)
	}
