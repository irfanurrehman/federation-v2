	...
	for key := range typeConfigFixtures {
		...
		Describe(fmt.Sprintf("%q", typeConfigName), func() {
			It("should be created, read, updated and deleted successfully", func() {
				...
				if err != nil {
					tl.Fatalf("Error retrieving typeconfig %q: %v", typeConfigName, err)
				}
				...
				testObjectsFunc := func(namespace string, clusterNames []string) (*unstructured.Unstructured, []interface{}, error) {
					...
					if typeConfig.GetTarget().Kind == util.NamespaceKind {
						fedObj, err := common.NewTestObject(typeConfig, namespace, clusterNames, fixture)
						if err != nil {
							return nil, nil, err
						}
						return fedObj, nil, nil
					}
					targetObject, err := common.NewTestTargetObject(typeConfig, namespace, fixture)
					if err != nil {
						return nil, nil, err
					}
					overrides, _, err := common.OverridesFromFixture(clusterNames, fixture)
					if err != nil {
						return nil, nil, err
					}
